...
void serviceMotion() {
    ...
    case MS_REARM_WAIT:
        if (millis() - rearmStartMs >= 2000) {
            ...
            capturePointIdx = 0;
            if (homeLimitHit()) {
                // already at home — skip straight to first scan step
                motionState = MS_SCAN_STEP;
    ...
    case MS_HOMING:
        // Drive toward home switch; stop the instant it trips, then
        // immediately command the first scan step.
        if (homeLimitHit()) {
            motor.MoveStopAbrupt();
            motor.PositionRefSet(0);
            capturePointIdx = 0;
            setMsg("HOMED - first step");
            Serial.println("[SEQ1] home limit hit -> first scan step");
            motionState = MS_SCAN_STEP;
            motor.Move(SCAN_STEP_STEPS, StepGenerator::MOVE_TARGET_ABSOLUTE);
...
void updateStatusRegisters() {
    ...
    s.b.homed        = (motionState == MS_SCAN_STEP  ||
                        motionState == MS_SETTLE     ||
                        motionState == MS_AT_CAPTURE ||
                        motionState == MS_RETURNING  ||
